...
void traverse(size_t start, bool twice, int& counter)
{
    if (start == find("end")) {
        counter++;
        return;
    ...
    for (size_t i = 0; i < unique.size(); ++i) {
        if (connections[start][i]) {
            if (!islower(unique[i][0])) {
                traverse(i, twice, counter);
            } else if (!visited[i]) {
                visited[i] = true;
                traverse(i, twice, counter);
                visited[i] = false;
            } else if (!twice && i != find("start")) {
                traverse(i, true, counter);
            }
        }
    }
}
...
int main()
{
    ...
    traverse(find("start"), false, counter);
